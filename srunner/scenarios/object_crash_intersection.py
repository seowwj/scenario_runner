...
def get_opponent_transform(added_dist, waypoint, trigger_location):
    ...
    transform = carla.Transform(location+carla.Location(z=2), carla.Rotation(yaw=orientation_yaw))
...
class VehicleTurningRoute(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._other_transforms = []
        self._bike_number = 20
    ...
    def _initialize_actors(self, config):
        ...
        for _ in range(0,self._bike_number):
            ...
            right_waypoint, _ = get_right_driving_lane(waypoint)
            ...
            while True:
                ...
                try:
                    other_actor_transform = get_opponent_transform(1, right_waypoint, self._trigger_location)
                    first_vehicle = CarlaDataProvider.request_new_actor(
                        'vehicle.diamondback.century', other_actor_transform)
                ...
                # Move the spawning point a bit and try again
                    ...
                    print(" Base transform is blocking objects ", other_actor_transform)
                    added_dist += 0.5
            ...
            self._other_transforms.append(other_actor_transform)
    ...
    def _create_behavior(self):
        ...
        if self._ego_route is not None:
            trigger_distance = InTriggerDistanceToLocationAlongRoute(self.ego_vehicles[0],
                                                                     self._ego_route,
                                                                     self._other_transforms[0].location,
                                                                     bycicle_start_dist)
        else:
        ...
        for i in range(0, self._bike_number):
            scenario_sequence.add_child(ActorTransformSetter(self.other_actors[i], self._other_transforms[i],
                                                            name='TransformSetterTS4'))
            scenario_sequence.add_child(HandBrakeVehicle(self.other_actors[i], True))
            scenario_sequence.add_child(Idle(0.5))
